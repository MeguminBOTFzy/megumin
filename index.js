...
const starts = async (kurr = new WAConnection()) => {
	console.log(color('SOURCE CODE INI DIJAGA DAN DI LINDUNGI OLEH FAJAR ALFARIZI\n\n', 'orange'), color('\n======TERIMKASIH BANYAK KEPADA======\n•Aldy Fauzy\n•Kemol\n•ADI\n•KurrXd\n•YUDHA\n•PAJARn•Dcode Denpa\n•Finx\n•SELURUH SUBSCRIBERKU', 'yellow'))
	console.log(color('\n\nJANGAN DI JUAL  BRO😑\nKALAU ADA YG JUAL SURUH CHAT ©mhmdfjralfarizi_\nWA 6281333782061', 'pink'))
    kurr.logger.level = 'warn'
    kurr.version = [2, 2143, 3]
    kurr.browserDescription = [ 'Fajar Alfarizi', 'Chrome', '3.0' ]
    
    kurr.on('qr', () => {
        console.log(color('[','white'), color('!','red'), color(']','white'), color(' Scan bang'))
    })

    fs.existsSync('./QRnya.json') && kurr.loadAuthInfo('./QRnya.json')
    kurr.on('connecting', () => {
        start('2', 'Connecting...')
    })
    kurr.on('open', () => {
    ...
    await kurr.connect({timeoutMs: 30*1000})
        fs.writeFileSync('./QRnya.json', JSON.stringify(kurr.base64EncodedAuthInfo(), null, '\t'))
...
if (!welkom.includes(anu.jid)) return
		try {
		    fkontakk = { key: {
		    fromMe: false,
		    participant: `0@s.whatsapp.net`, ...(anu.jid ? { remoteJid: '6281333782061-1617740713@g.us' } : {})
		    },
		    message: {
...
"inviteCode": "K1eU4aZZ8k7BO6eNndwvY1",
"groupName": "𝐹𝑎𝑗𝑎𝑟 𝐴𝑙𝑓𝑎𝑟𝑖𝑧𝑖",
"jpegThumbnail": fs.readFileSync('./media/thumb.jpg'),
"caption": `Fajar Alfarizi`
}
